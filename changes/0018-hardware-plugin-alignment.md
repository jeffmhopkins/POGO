...
| 6 — Triple BP + Dist | 🚧 stages 1–4 done | 🚧 | G5+G6a | Split into 7 sections. **S1:** F_REF→400Hz, 2-pole. **S2:** DIST→SVF reorder. **S3:** BP3 selector (CD4053 U81 + ALT_L_DET). **S4:** per-band TILT built (was entirely unbuilt — no knob component existed): +3 TL072 (U82-84 summer+inverter/band) + 3 TILT knobs (RV48-50) + MOD_BPn_TILT CV ×0.22 (470k); master+knob+CV → ±total → expo bases; shared master inverter removed (BP_NTILT gone, U27-A freed). **Pending:** S5 variable DRIVE, S6 CLIP drivers, S7 two-scaler mix. |
| 7 — HP Filter | 🔲 | 🔲 | — | |
| 8 — LP Filter 2 | 🔲 | 🔲 | — | |
