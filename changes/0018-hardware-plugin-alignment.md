...
| 6 — Triple BP + Dist | 🚧 stages 1–2 done | 🚧 | — | Split into 7 sections (Scheme B). **Stage 1:** F_REF→400Hz all bands (caps→68nF), 2-pole confirmed. **Stage 2:** DIST→SVF reorder done (re-pointed nets: `LP1_OUT/BP3IN→distN→BPn_DISTOUT→svfN→BPn_BANDOUT/BP3_TAP→mix`; BP3 tap now post-SVF). **Pending:** S3 BP3 selector (← block-4 `ALT_VCA_OUT`), S4 TILT ×0.22, S5 variable DRIVE, S6 CLIP drivers, S7 two-scaler mix. |
| 7 — HP Filter | 🔲 | 🔲 | — | |
| 8 — LP Filter 2 | 🔲 | 🔲 | — | |
