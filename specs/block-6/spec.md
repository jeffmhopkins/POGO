...
> ⚠️ **SUPERSEDED (pre-2026-05-27 theory)** — This section predates change 0018. Where it
> conflicts with §1, the Stage-alignment notes above (two-scaler BP_BYPASS+BP_WET mix; **per-band**
> `BP{n}_DIST_MODE`; distortion **before** the SVF; F_REF 400 Hz; per-band variable DRIVE VCA),
> or the `specs/block-6-{svf1,svf2,svf3,dist1,dist2,dist3,mix}/*.nets.yaml`, those are authoritative.
> In particular the "global BP_DIST switch" and "BP_MIX crossfade (default 0.5)" language below is
> stale — mode is per-band and the output uses two independent scalers (both default 1.0).
...
Three modes, selected **per band** by `BP{n}_DIST_MODE` (0/1/2) — BP1/BP2/BP3 independent:
...
```
**(Superseded — see §1 / Stage-7 note.)** The current design uses two independent scalers: `BP_BYPASS` scales the dry LP1 band and `BP_WET` scales the wet sum, both default **1.0** (`bpOut = clamp(band·BYPASS + wetSum·WET, ±12)`). There is no single "mix" crossfade and no 0.5 default.
