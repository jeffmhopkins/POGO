...
KiCad 7 `.kicad_sch` files are generated from those specs. PCB layout is done in KiCad
directly using a hybrid approach: critical signal paths and power rails are hand-routed;
remaining traces are completed by an auto-router.
...
PCB layout uses KiCad native workflow with a hybrid routing strategy:

1. Open `pogo-control-board.kicad_sch` in KiCad 7, then **Tools → Update PCB from Schematic**
   to push the netlist into the `.kicad_pcb` file
2. Component placement is generated by `kicad/generate_control_pcb.py` (reads
   `kicad/layouts/control-board.json` for positions derived from `specs/panel-design/panel.svg`)
3. **Hand-route first**: power rails (+12V, -12V, GND planes), I_abc signal traces (critical
   for noise — see noise-audit.md H2/H3), and any traces where adjacency matters
4. **Auto-route remaining**: run the KiCad built-in auto-router (or external: FreeRouting)
   on all remaining unrouted nets
5. Run DRC after auto-routing; fix any violations manually

Footprints for all parts are committed to `kicad/footprints/` sourced from the KiCad
official footprint library and manufacturer datasheets. No footprint geometry is hand-coded
from memory — see commit history for source citations.
