...
4. Bipolar attenuverter pot (50 kΩ linear, centre-detent; change 0020 §H, was 10 kΩ): wiper voltage is 0 V at noon,
...
**Distribution buffer:** V_modbus drives the 18 destination nodes (each a 50 kΩ attenuverter pot
plus its 47 kΩ −V_src inverter input — the ×5 hi-Z load of change 0020 §H) plus the raw VCA normal.
Per §H the net bus load is **~11 mA** at V_modbus = ±10 V (down from the pre-§H ~18 mA on the old
10 kΩ pots). To achieve reliable ±10 V swing at this load, MB_PROC_A uses both of its two spare
sections (halves C and D) wired in parallel as a unity-gain buffer — each half drives ~5.5 mA, well
within TL074 limits. A 47 Ω series resistor on each output before the join prevents oscillation from
unequal saturation times.
