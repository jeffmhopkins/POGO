...
struct LFO {
	...
	float process(float speedParam, float sampleTime) {
		...
		float raw = (phase < 0.5f) ? (4.f * phase - 1.f) : (3.f - 4.f * phase);
		// One-pole LP at 10× rate: models integrator slew / Schmitt rounding at peaks
		float tau   = 1.f / (10.f * speedHz + 1e-6f);
		float alpha = sampleTime / (tau + sampleTime);
		prevOut += alpha * (raw - prevOut);
		return prevOut;
	}

	void reset() { phase = prevOut = 0.f; }
};
