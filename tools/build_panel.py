...
def _propagate_svg_fills(svg_path: Path) -> None:
    """Propagate fill colors from <g style="fill:..."> down to child <path> elements.

    Inkscape's --export-text-to-path wraps converted glyphs in a <g> with the fill
    color in the group's style attribute, but leaves individual <path> elements without
    an explicit fill. NanoSVG (VCV Rack's SVG renderer) may not inherit fills from
    parent group styles, making labels invisible. This step ensures every <path> has
    an explicit fill attribute.
    """
    ...
    def _walk(elem: ET.Element, inherited: str | None) -> None:
        # Determine effective fill for this element
        fill: str | None = inherited
        if "fill" in elem.attrib:
            fill = elem.attrib["fill"]
        sf = _fill_from_style(elem.attrib.get("style", ""))
        if sf:
            fill = sf

        tag = elem.tag.split("}")[-1]  # strip namespace prefix
        if tag == "path":
            has_fill = "fill" in elem.attrib or _fill_from_style(elem.attrib.get("style", ""))
            if not has_fill and fill and fill not in ("inherit", "none"):
    ...
    # Re-serialise, preserving the standard generated-file header
    body = ET.tostring(root, encoding="unicode")
    svg_path.write_text(
        '<?xml version="1.0" encoding="UTF-8" standalone="no"?>\n'
        "<!-- GENERATED by tools/build_panel.py — edit tools/panel-data.yaml -->\n\n"
        + body,
        encoding="utf-8",
    )
